...
datNames = ["WT","\Delta{\itmexZ}","\Delta{\itmexY}"];
...
for k = 1:3
    normCheck = mean(g(k,:,end-minusFrame:end),3,"omitnan");
...
% growth data column concatenated with category column
dat = [mean(g(1,:,idx2-minusFrame:idx2),3,"omitnan")' ones(size(m,2),1)];
dat = [dat;mean(g(2,:,idx2-minusFrame:idx2),3,"omitnan")' 2*ones(size(m,2),1)];
dat = [dat;mean(g(3,:,idx2-minusFrame:idx2),3,"omitnan")' 3*ones(size(m,2),1)];
...
%l=legend([b1,b2,b3],datNames,'Location','northeast','AutoUpdate','off');
set(gca,'XTick',[1 2 3],'XTickLabels',["WT","\Delta{\itmexZ}","\Delta{\itmexY}"])
...
if pANOVA < 0.05
    ...
    newY = ritzStar_g(comparisons,pPairwise,1.1);
